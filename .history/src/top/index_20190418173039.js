...
class Top extends Component {
    ...
    render() {
        ...
        if (list) {
            movie_list = list.map((value, index) => {
                return ( <
                    ...
                    onClick = {
                        (e) => this.detailHandler(value.id, e)
                    } >
                    <
            ...
            page_list = items.map((value, index) => {
                return ( <
                    ...
                    a href = "javascript:void(0);"
                    className = {
                        this.state.pageCurrent === value ? 'active' : 'ban'
                    }
                    onClick = {
                        (e) => this.pageHandler(value, e)
                    } > {
                        value
                    } <
                    /a> <
        ...
        return ( <
            ...
            <
            ul className = {
                this.state.pageTotal >= 1 ? 'active' : 'hide'
            } >
            <
            Spin spinning = {
                this.state.list ? false : true
            }
            size = "large"
            className = "movie-spin" > {
                page_list
            } <
            /Spin> <
            div className = "clear" > < /div> <
